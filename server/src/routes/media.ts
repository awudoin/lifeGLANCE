...
mediaRouter.post(
  ...
  async (request: Request, response: Response, next: NextFunction) => {
    try {
      const milestoneId: string | undefined = typeof request.body.milestoneId === "string" ? request.body.milestoneId : undefined;
      const kind: string | undefined = typeof request.body.kind === "string" ? request.body.kind : undefined;
...
mediaRouter.get("/media/:id", async (request: Request, response: Response, next: NextFunction) => {
  try {
    ...
    if (!mediaId) {
      response.status(400).json({ error: "invalid_param", message: "Media id is required." });
      return;
    }

    const record = await findMediaRecord(mediaId);
    if (!record) {
      response.status(404).json({ error: "not_found", message: "Media file was not found." });
      return;
    }

    const absolutePath: string = resolveMediaAbsolutePath(record.storagePath);
    if (!fs.existsSync(absolutePath)) {
      response.status(404).json({ error: "not_found", message: "Media file is missing on disk." });
      return;
    }
...
mediaRouter.delete("/media/:id", async (request: Request, response: Response, next: NextFunction) => {
  try {
    const mediaId = getSingleParam(request.params.id);
    if (!mediaId) {
      response.status(400).json({ error: "invalid_param", message: "Media id is required." });
