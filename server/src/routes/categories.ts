...
categoryRouter.get("/categories", async (_request: Request, response: Response, next: NextFunction) => {
  try {
...
categoryRouter.put("/categories", async (request: Request, response: Response, next: NextFunction) => {
  try {
    if (!Array.isArray(request.body) || !request.body.every(isCategoryInput)) {
      response.status(400).json({ error: "invalid_body", message: "Category payload must be an array of categories." });
      return;
    }
