...
function isRestoreMilestoneBody(value: unknown): value is RestoreMilestoneBody {
  ...
  const hasValidCreatedAt = candidate.createdAt === undefined || typeof candidate.createdAt === "string";
  const hasValidUpdatedAt = candidate.updatedAt === undefined || typeof candidate.updatedAt === "string";

  return hasValidId && hasValidCreatedAt && hasValidUpdatedAt;
}

milestoneRouter.get("/milestones", async (_request: Request, response: Response, next: NextFunction) => {
  try {
...
milestoneRouter.post("/milestones", async (request: Request, response: Response, next: NextFunction) => {
  try {
    if (!isUpsertMilestoneBody(request.body)) {
      response.status(400).json({ error: "invalid_body", message: "Milestone payload is invalid." });
      return;
    }
...
milestoneRouter.put("/milestones/:id", async (request: Request, response: Response, next: NextFunction) => {
  try {
    const milestoneId = getSingleParam(request.params.id);
    if (!milestoneId) {
      response.status(400).json({ error: "invalid_param", message: "Milestone id is required." });
      return;
    }
    ...
    if (!milestone) {
      response.status(404).json({ error: "not_found", message: "Milestone was not found." });
      return;
    }
...
milestoneRouter.delete("/milestones/:id", async (request: Request, response: Response, next: NextFunction) => {
  try {
    const milestoneId = getSingleParam(request.params.id);
    if (!milestoneId) {
      response.status(400).json({ error: "invalid_param", message: "Milestone id is required." });
...
milestoneRouter.delete("/recurrences/:recurrenceId", async (request: Request, response: Response, next: NextFunction) => {
  try {
    const recurrenceId = getSingleParam(request.params.recurrenceId);
    if (!recurrenceId) {
      response.status(400).json({ error: "invalid_param", message: "Recurrence id is required." });
      return;
    }
...
milestoneRouter.post("/milestones/restore", async (request: Request, response: Response, next: NextFunction) => {
  try {
    if (!Array.isArray(request.body) || !request.body.every(isRestoreMilestoneBody)) {
      response.status(400).json({ error: "invalid_body", message: "Restore payload must be an array of milestones." });
      return;
    }
