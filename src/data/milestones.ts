...
async function syncLocalCacheWithRemote(remoteItems: ServerMilestone[], localItems: FrontendMilestone[]): Promise<FrontendMilestone[]> {
...
export async function loadMilestones(): Promise<FrontendMilestone[]> {
  ...
  try {
    ...
    if (bootstrap.milestones.length === 0 && localItems.length > 0) {
      console.warn("Backend milestone store is empty; falling back to local metadata cache until migration is implemented.");
      return localItems;
    }
...
export async function updateMilestone(
  ...
  const nextDate = updates.date instanceof Date
    ? updates.date
