...
export function playChime() {
    playPianoNote(523.25, 0.1, 1.1);
    setTimeout(() => playPianoNote(659.25, 0.07, 0.95), 70);
    setTimeout(() => playPianoNote(783.99, 0.05, 0.8), 140);
}
...
function _fadeOutAmbient() {
    ambientActive = false;
    clearTimeout(melodyTimer ?? undefined);
    melodyTimer = null;
    ...
    nodes.forEach(({ osc, gainNd, lfo }) => {
        ...
        try {
            osc.stop(c ? c.currentTime + 1.3 : 0);
        } catch {}
        try {
            lfo.stop(c ? c.currentTime + 1.3 : 0);
        } catch {}
    });
}
